...
trait XhtmlUtils {
  ...
  def checkBox(name: String, checked: Boolean = false, value: String = "1", clazz: String = "", disabled: Boolean = false) =
    (checked, disabled) match {
      ...
      case (false, true) =>
        <input type="checkbox" id={name} class={clazz} name={name} value={value} disabled=""/>
      case (true, false) =>
        <input type="checkbox" id={name} class={clazz} name={name} value={value} checked=""/>
      case (true, true) =>
        <input type="checkbox" id={name} class={clazz} name={name} value={value} checked="" disabled=""/>
    }

  def radio(name: String, value: String, checked: Boolean = false) =
    if (checked)
